...
class Network:
    ...
    def SGD(self , training_data , epochs , mini_batch_size , eta , test_data=None, plot_cost=False):
        """ training data is in the form (x, y) where x is the input and y is the desired output. x and y are both COLUMN vectors
        """
        if test_data:
        ...
        for epoch in range(epochs):
            ...
            for mini_batch in mini_batches:
                mini_batch_costs = self.update_mini_batch(mini_batch, eta, plot_cost=plot_cost)
                
                if plot_cost:
                    epoch_costs.extend(mini_batch_costs)
            ...
            if plot_cost:
                ...
                print(f"Average Cost in epoch {epoch}: {sum(epoch_costs)/n} ")

    def update_mini_batch(self, mini_batch, eta, plot_cost=False):
        ...
        for x, y in mini_batch:
            ...
            if plot_cost:
                costs.append(self.quadratic_cost(self.feedforward(x), y))
    ...
    def quadratic_cost(self, activations, correct_values):
        # cost = sum([abs(a-y) for a, y in zip(activations, correct_values)])
        cost = sum([(a-y)**2 for a, y in zip(activations, correct_values)])
        cost *= 0.5
        return cost
    ...
    def scramble_parameters(self):
        self.biases = [np.random.randn(y, 1) for y in self.sizes[1:]] # a list of 1d arrays 
        self.weights = [np.random.randn(y, x) for x, y in zip(self.sizes[:-1], self.sizes[1:])] # a list of 2d matricies. j, k still follows w_j,k still is the weight to neuron j from neuron k.
